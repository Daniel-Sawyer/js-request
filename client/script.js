...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/getthis`)
    .then((res) =>{
        console.log(res.data);
...
const queryTest = () => {
    axios.get(`${baseURL}/query-test?tester & nice `)
    .then ((res) => {
        console.log(res.data);
    })
}
...
/*
    ...
    just says i sent more then one and the names of them 
*/
...
*/
createFoodCard = (food) =>{
    let foodcard = document.createElement(`p`)
    foodcard.innerHTML = `<p>${food}<p>`
